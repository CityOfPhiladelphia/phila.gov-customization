	...
	oneApp.TextView = oneApp.SectionView.extend({
		events: function() {
			return _.extend({}, oneApp.SectionView.prototype.events, {
				'change .ttfmake-phila_gov-columns' : 'handleColumns'
			});
		},
		...
		handleColumns : function (evt) {
			...
			var columns = $(evt.target).val(),
				$stage = $('.ttfmake-phila_gov-columns-stage', this.$el);

			$stage.removeClass('ttfmake-phila_gov-columns-1 ttfmake-phila_gov-columns-2 ttfmake-phila_gov-columns-3 ttfmake-phila_gov-columns-4');
			$stage.addClass('ttfmake-phila_gov-columns-' + parseInt(columns, 10));
		}
	});
		...
		$selector.sortable({
			...
			start: function (event, ui) {
				...
				var $item = $(ui.item.get(0)),
					$stage = $item.parents('.ttfmake-phila_gov-columns-stage'),
					addClass = '';

				// If text item, potentially add class to stage
				if ($item.hasClass('ttfmake-phila_gov-column')) {
			...
			stop: function (event, ui) {
				var $item = $(ui.item.get(0)),
					...
					$columnsStage = $item.parents('.ttfmake-phila_gov-columns-stage'),
					$orderInput = $('.ttfmake-phila_gov-columns-order', $stage),
				...
				i = 1;
				$('.ttfmake-phila_gov-column', $stage).each(function(){
					$(this)
						.removeClass('ttfmake-phila_gov-column-position-1 ttfmake-phila_gov-column-position-2 ttfmake-phila_gov-column-position-3 ttfmake-phila_gov-column-position-4')
						.addClass('ttfmake-phila_gov-column-position-' + i);
					i++;
				});
