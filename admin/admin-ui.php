...
class phila_role_administration {
  //we can only run this role-check after plugins have been loaded
    ...
    public function check_if_user_logged_in(){
      //set up the args for the categories
      ...
      $categories = get_categories( $categories_args );
        if ( is_user_logged_in() ){
          ...
          $key = 'wp_capabilities';
          $roles = get_user_meta($user, $key, true);
          ...
          //set up a new array for matches between $roles & $cat_slugs
          $matches = [];
          foreach($cat_slugs as $key) {
              if(array_key_exists($key, $roles)) {
                  $matches[$key] = $roles[$key];
              }
          }
          //our matches are our driver!
          var_dump($matches);
        }
    }
}

$phila_role_administration_var = new phila_role_administration();
